...
namespace DATA {
    ...
    bool checkOrOpen( FILE*& p_f, const char* p_path, u8& p_lastLang, u8 p_language ) {
        if( p_f == nullptr || p_language != p_lastLang ) {
            ...
            p_f        = openBank( p_path, p_language, ".strb" );
    ...
    FILE* openSplit( const char* p_path, u16 p_value, const char* p_ext, u16 p_maxValue,
                     const char* p_mode ) {
        if( p_maxValue < 10 * MAX_ITEMS_PER_DIR ) {
            snprintf( TMP_BUFFER, 99, "%s%d/%d%s", p_path, p_value / MAX_ITEMS_PER_DIR, p_value,
                      p_ext );
        } else if( p_maxValue < 100 * MAX_ITEMS_PER_DIR ) {
            snprintf( TMP_BUFFER, 99, "%s%02d/%d%s", p_path, p_value / MAX_ITEMS_PER_DIR, p_value,
                      p_ext );
        } else {
            snprintf( TMP_BUFFER, 99, "%s%03d/%d%s", p_path, p_value / MAX_ITEMS_PER_DIR, p_value,
                      p_ext );
        }
